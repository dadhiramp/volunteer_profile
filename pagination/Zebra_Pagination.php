...
class Zebra_Pagination
{
    ...
    function Zebra_Pagination()
    {
        ...
        $this->variable_name('page_id');
        
        // default method for page propagation
        $this->method('get');
        
        // set the base url
        $link_arr = explode('/', $_SERVER['SCRIPT_NAME']);
        $link_pos = count($link_arr)-1;
        $this->base_url($link_arr[$link_pos]);
    ...
    function base_url($base_url = '')
    {
       ...
       // $this->base_url = ($base_url == '' ? $_SERVER['REQUEST_URI'] : $base_url);
		 $this->base_url = ($base_url == '' ? $_SERVER['QUERY_STRING'] : $base_url);
    ...
    function get_page()
    {
        ...
        // if page propagation is done through GET
            ...
            // if the current page is set in $_GET
    ...
    function render($return_output = false)
    {
        ...
        if ($this->_total_pages > $this->selectable_pages) {
            ...
            if ($this->page == 1) {
                ...
                $output .= '<a href="javascript:void(0)" class="navigation left disabled">previous page</a>';

            // if we're not on the first page
            } else {

                // we'll use the usual link
                $output .= '<a href="' . $this->_build_uri($this->page - 1) . '" class="navigation left">previous page</a>';
        ...
        // if the total number of pages is greater than the number of selectable pages
            ...
            if ($this->_total_pages > $this->selectable_pages) {
                ...
                if ($this->page == $this->_total_pages) {
                    ...
                    // we want a "disabled" link
                    $output .= '<a href="javascript:void(0)" class="navigation right disabled">next page</a>';

                // if we're not on the last page
                } else {

                    // we'll use the usual link
                    $output .= '<a href="' . $this->_build_uri($this->page + 1) . '" class="navigation right">next page</a>';
